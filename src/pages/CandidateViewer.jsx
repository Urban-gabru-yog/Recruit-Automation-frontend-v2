...
const CandidateViewer = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await axios.get(
        `https://recruit-automation-backend-v2.onrender.com/api/jobs/${job_id}`
      );
      setJob(res.data.job);
  ...
  const handleAutoScore = async () => {
    ...
    try {
      await axios.post(
        "https://recruit-automation-backend-v2.onrender.com/api/n8n/score-candidates"
      );
  ...
  const handleStatusUpdate = async (id, status) => {
    ...
    try {
      await axios.post(
        `https://recruit-automation-backend-v2.onrender.com/api/form/update-status/${id}`,
        { hr_status: status }
      );
      alert(
        `Candidate marked as ${
