...
const HiringForm = () => {
  ...
  useEffect(() => {
    const fetchJobData = async () => {
      ...
      try {
        ...
        if (!jobData) {
          setError("Job not found or invalid link.");
          return;
        }
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    if (!job) {
      setError("Job details are not loaded yet.");
      return;
    }
    ...
    try {
      ...
      setError(
        "Failed to submit application. Please ensure all fields are filled and try again."
      );
      console.error(
        "Submission failed:",
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 via-white to-cyan-50 px-4">
        <div className="text-center">
          ...
          <h3 className="text-2xl font-bold text-gray-800 mb-2">Loading Job Details</h3>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 px-4">
        <div className="bg-white/90 backdrop-blur-xl p-10 rounded-3xl shadow-2xl border border-red-100 max-w-lg w-full text-center transform hover:scale-105 transition-transform duration-300">
          ...
          <h2 className="text-3xl font-bold text-red-700 mb-4">Position Closed</h2>
          <p className="text-gray-700 text-lg leading-relaxed">
            We appreciate your interest! Unfortunately, this job posting is no longer accepting applications.
          </p>
        ...
        <div className="bg-white/90 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-white/20 transform hover:scale-[1.02] transition-all duration-300">
          ...
          <div className="flex flex-col items-center mb-10">
            <div className="relative mb-6">
              <div className="w-28 h-28 bg-gradient-to-br from-indigo-100 to-cyan-100 rounded-2xl flex items-center justify-center shadow-lg">
                <img src={UrbangabruLogo} alt="Urbangabru Logo" className="w-20 h-20 object-contain" />
              </div>
            ...
            <div className="flex items-center mt-3 px-4 py-2 bg-gradient-to-r from-indigo-50 to-cyan-50 rounded-full">
              ...
              <p className="text-indigo-700 font-semibold text-sm">Career Application</p>
            </div>
          </div>
          ...
          <div className="text-center mb-8 p-6 bg-gradient-to-r from-indigo-50 to-cyan-50 rounded-2xl border border-indigo-100">
            <h2 className="text-2xl font-bold text-gray-800 mb-2">
              Apply for <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-cyan-600">{job?.position || "—"}</span>
            </h2>
            <p className="text-gray-600 font-medium">
              Join our <span className="text-indigo-600 font-semibold">{job.team}</span> team and make an impact!
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {customQuestions.map((q, idx) => (
              <div key={idx} className="relative">
                <label htmlFor={q.name} className="block text-sm font-semibold text-gray-700 mb-2">
                  {q.label}
                </label>
                <textarea
                  id={q.name}
            ...
            <div className="relative">
              <label htmlFor="willingToRelocate" className="block text-sm font-semibold text-gray-700 mb-2">
            ...
            <div className="relative">
              <label htmlFor="resume-upload" className="block text-sm font-semibold text-gray-700 mb-2">
                Resume Upload
              </label>
              <div className="relative">
                <input
              ...
              {resume && (
                <div className="mt-3 p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
                  <div className="flex items-center">
                    ...
                    <span className="text-sm text-green-700 font-medium">{resume.name}</span>
                  </div>
                </div>
              )}
            </div>
